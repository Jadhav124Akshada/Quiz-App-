...
const cppQuestions = [
  ...
  {
    question:
      "Which concept allows the same function to behave differently based on input?",
  ...
  {
    question:
      "Which of the following allows access to private members of a class?",
    options: [
      "Friend function",
      "Constructor",
      "Destructor",
...
const dbmsQuestions = [
  ...
  {
    question:
      "Which language is used to create and modify database structures?",
  ...
  {
    question:
      "Which of the following is used to provide access control in DBMS?",
    options: ["DML", "DCL", "DDL", "TCL"],
    answer: "DCL",
  },
  {
  ...
  {
    question:
      "Which command is used to delete all rows from a table without logging?",
    ...
    <ul class="option-container">
      ${question.options
        .map(
          (option, i) => `
        <li>
          <label for="question-${index}-option-${i}">
            <input type="radio" id="question-${index}-option-${i}" name="question-${index}" value="${option}"> ${option}
          </label>
        </li>
      `
        )
...
// Function to check the answer for a specific question and highlight correct and wrong options.
...
const checkAnswer = (questions, index) => {
  ...
  // to take correct option of the question and hightlight it
  const correctOption = document.querySelector(
    `input[value="${questions[index].answer}"]`
  );

  const correctOptionLabel = document.querySelector(
    `label[for="${correctOption.id}"]`
  );
  ...
  if (selectedOption.value === questions[index].answer) {
    ...
    selectedLabel.style.color = "#b71c1c";
    return false;
  }
};
...
const calculateTotalScore = (questions) => {
  ...
  <h1>Your total score is: ${score} out of ${questions.length}</h1>
  `;
  totalScoreContainer.style.color =
    score === questions.length ? "green" : "pink";
};
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (sectionId) {
    ...
    // Restrict selection to only one
    document.querySelectorAll('input[type="radio"]').forEach((radio) => {
      radio.addEventListener("click", function (e) {
        const label = document.querySelector(`label[for="${radio.id}"]`);
        label.style.color = "#8b318e";
        const name = this.name;

        // progress bar functionality
        const anyChecked =
          document.querySelectorAll(`input[name='${name}']:checked`).length > 1;
        ...
        const unattemptedCount = document.getElementById("unattempted-count")
          ...
          // only increment if this is the first click on this group
        if (!anyChecked) {
          count++;
          console.log(count)
          ...
          progressText.textContent = `Progress : ${count} / 10`
          attemptedCount.textContent = `${count}`
          unattemptedCount.textContent = `${10 - count}`
          progressPercent.textContent = `${count*10}%`;
        }
